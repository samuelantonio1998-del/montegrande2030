...
import { Trash2, Recycle, TrendingDown, Euro, BarChart3 } from 'lucide-react';
import { BarChart, Bar, XAxis, YAxis, CartesianGrid, Tooltip, ResponsiveContainer, Cell } from 'recharts';
import { cn } from '@/lib/utils';
import { mockWeeklyWaste } from '@/lib/buffet-data';

export default function Desperdicio() {
  const totalWaste = mockWeeklyWaste.reduce((sum, d) => sum + d.totalWasteKg, 0);
  const totalReused = mockWeeklyWaste.reduce((sum, d) => sum + d.totalReusedKg, 0);
  const totalLoss = mockWeeklyWaste.reduce((sum, d) => sum + d.estimatedLoss, 0);
  const totalSavings = mockWeeklyWaste.reduce((sum, d) => sum + d.estimatedSavings, 0);
  const totalProduced = mockWeeklyWaste.reduce((sum, d) => sum + d.totalProducedKg, 0);

  const sortedByWaste = [...mockWeeklyWaste].sort((a, b) => b.wastePercentage - a.wastePercentage);
  const sortedBySavings = [...mockWeeklyWaste].sort((a, b) => b.estimatedSavings - a.estimatedSavings);

  const chartData = mockWeeklyWaste.map(d => ({
  ...
  return (
    <div className="space-y-8">
      <div>
        ...
        <p className="mt-1 text-muted-foreground">Relatório semanal · 18–24 Março 2026</p>
      </div>
      ...
      <div className="grid grid-cols-1 gap-6 lg:grid-cols-2">
        ...
        >
          ...
          <div className="space-y-3">
            {sortedByWaste.slice(0, 3).map((item, i) => (
              <div key={item.dishName} className="flex items-center gap-4 rounded-lg bg-destructive/5 p-4">
                ...
                <div className="flex-1">
                  ...
                  <p className="text-xs text-muted-foreground">
                    {item.totalWasteKg}kg desperdiçados de {item.totalProducedKg}kg
                  </p>
                </div>
                <div className="text-right">
                  <p className="text-sm font-semibold text-destructive">{item.wastePercentage}%</p>
            ...
            {sortedBySavings.filter(i => i.estimatedSavings > 0).slice(0, 3).map((item, i) => (
              <div key={item.dishName} className="flex items-center gap-4 rounded-lg bg-success/5 p-4">
                ...
                <div className="flex-1">
                  <p className="font-medium text-foreground">{item.dishName}</p>
                  <p className="text-xs text-muted-foreground">
                    {item.totalReusedKg}kg reaproveitados
                  </p>
                </div>
                <div className="text-right">
      ...
      >
        ...
        <div className="overflow-x-auto">
          <table className="w-full text-sm">
            ...
            <tbody>
              {mockWeeklyWaste.map(item => (
                <tr key={item.dishName} className="border-b border-border/50">
                  <td className="py-3 font-medium text-foreground">{item.dishName}</td>
                  <td className="py-3 text-right text-muted-foreground">{item.totalProducedKg}kg</td>
                  <td className="py-3 text-right text-destructive">{item.totalWasteKg}kg</td>
                  <td className="py-3 text-right text-success">{item.totalReusedKg}kg</td>
              ...
              <tr className="font-semibold">
                ...
                <td className="pt-3 text-right">
                  <span className="rounded-full bg-muted px-2 py-0.5 text-xs">
                    {((totalWaste / totalProduced) * 100).toFixed(1)}%
                  </span>
                </td>
