  ...
  try {
    ...
    if (action === "pessoa_update") {
      ...
      if (funcionario_id) {
        await supabase
          ...
          .update({
            ...(nome ? { nome } : {}),
            ...(role_id ? { role_id, role: papel?.chave ?? "utilizador" } : {}),
            unidade_id: unidade_id ?? null,
          })
    ...
    if (action === "pessoa_set_pin") {
      ...
      if (!fid) {
        ...
        const { data: inserted, error: insErr } = await supabase
          .from("funcionarios")
          .insert({ nome, role: papel?.chave ?? "utilizador", role_id: role_id ?? null, unidade_id: unidade_id ?? null, ativo: true })
          .select("id")
          .single();
    ...
    if (action === "pessoa_add_conta") {
      ...
      const { data: created, error } = await supabase.auth.admin.createUser({
        ...
        password,
        email_confirm: true,
        app_metadata: { nome: nome ?? null, unidade_id: unidade_id ?? null, funcionario_id, role: papel?.chave ?? "utilizador" },
        user_metadata: { nome: nome ?? null },
      });
